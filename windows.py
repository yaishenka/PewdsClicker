...
class MainGameWindow(GameWindow):
    ...
    def init_counters(self):
        self.scores = get_pewds_subs_count()
        self.money = 0
    ...
    def money_per_sec(self):
        return (self.money_per_sec_ratio + self.bonuses[Bonus.BonusType.MONEY_PER_FRAME].get_bonus()) * self.scores
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if event.key == pygame.K_1:
                if self.money > self.bonuses[Bonus.BonusType.SUBS].get_price():
                    self.money -= self.bonuses[Bonus.BonusType.SUBS].get_price()
                    self.bonuses[Bonus.BonusType.SUBS].level += 1
            if event.key == pygame.K_2:
                if self.money > self.bonuses[Bonus.BonusType.MONEY_PER_FRAME].get_price():
                    self.money -= self.bonuses[Bonus.BonusType.MONEY_PER_FRAME].get_price()
